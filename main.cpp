...
void extend_cache(int bit_address, string mode, int trace_index, int level);
...
void Cache::access(int32_t bit_address, string mode, int trace_index)
{
    ...
    if (invalid_index != -1)
    {
        ...
        if (replc_policy == 0)
        {
            ...
            cache[idx][invalid_index] = Unit(1, 1, tag, bit_address, order_counts[idx]);
            order_counts[idx] += 1;
        }

        else if (replc_policy == 1)
        {
            pseudo_lru_trees[idx].tree_access(invalid_index);
            cache[idx][invalid_index] = Unit(1, 1, tag, bit_address);
        }
...
void extend_cache(int bit_address, string mode, int trace_index, int level)
{
    ...
    else if (level == 1)
    {
        ...
        ost = bit_address % l1.block_size;

        bit_address /= l1.block_size;

        idx = bit_address % l1.num_sets;

        tag = bit_address / l1.num_sets;
